...
'''
Module contains OpenSound Control implementation by Daniel Holth (this file,
see also comments below) plus an extension of the Server and Client
implementation specific for the MoveMe server application.
...
class CallbackManager(object):
	...
	def dispatch(self, message, source):
		...
		try:
			address = message[0]
			self.callbacks[address](message[2:], source)

		except KeyError, e:

			# address not found
			print 'no handler for', message, source

			# Raises error once or twice at start, maybe not
			# initialized yet? Skipping raise, just printing.
			# raise
